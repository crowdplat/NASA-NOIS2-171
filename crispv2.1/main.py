...
from image_scripts.train_image_model import train_image_model
...
def run(config):
    ############################ RUN IMAGE MODEL ###################################
    if(config["experiment_type"]=='image_only'):
        train_image_model(config)
    
    ############################ RUN IMAGE FEATURE WITH TABULAR DATA ###################################
    elif(config["experiment_type"]=='tabular_only' or config["experiment_type"]=='multimodal'):
